...
class TestSaveToFile(unittest.TestCase):
    ...
    def test_save_to_file_failure(self):
        ...
        with patch("builtins.open", mock_open()) as mocked_file:
            ...
            # Assertions
            mocked_file().__enter__.assert_called_once()  # Ensure __enter__ was called once
            mocked_file().__exit__.assert_called_once()   # Ensure __exit__ was called once
...
class TestDownloadHandling(unittest.TestCase):
    ...
    def test_download_handling_success_main_url(self, mock_save_to_file, mock_download):
        ...
        mock_response = MagicMock()
        mock_download.return_value = (True, mock_response)  # Simulate success for main URL
    ...
    def test_download_handling_success_alt_url(self, mock_save_to_file, mock_download):
        ...
        mock_response = MagicMock()
        mock_download.side_effect = [(False, None), (True, mock_response)]  # Main URL fails, alt URL succeeds
        mock_save_to_file.return_value = True  # Simulate successful save
